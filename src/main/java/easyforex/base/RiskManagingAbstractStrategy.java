...
import easyforex.util.RiskManagingtUtils;
...
public abstract class RiskManagingAbstractStrategy extends SingleInstrumentAbstarctStrategy {
	 ...
	 * Submits order using risk percent
	 * @param command
	 * @param stopLossPrice stop loss price - which risk will be calculated for
	...
	protected IOrder submitOrderCalculatingRisk(IEngine.OrderCommand command, double stopLossPrice, double takeProfitPrice) throws JFException {
		...
		double tradeAmount = RiskManagingtUtils.calculareTradeAmount(context, selectedInstrument, command, lastTick, stopLossPrice, riskPercent);
	 ...
	 * Submits order using risk percent
	 * @param command
	 * @param stopLossPips stop loss in pips - which risk will be calculated for
	...
	protected IOrder submitOrderCalculatingRiskForPips(IEngine.OrderCommand command, int stopLossPips, int takeProfitPips) throws JFException {
		double tradeAmount = RiskManagingtUtils.calculareTradeAmount(context, selectedInstrument, stopLossPips, riskPercent);
		ITick lastTick = getLastTick();
